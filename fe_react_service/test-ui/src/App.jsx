...
/**
 * Minimal React SPA for your microservices backend
 * Pages: Login → Dashboard → Test (single-question paging) → Submit → Dashboard (shows score)
 *
 * Endpoints (from your Postman collection):
 *  - POST  http://localhost:8003/api/auth/login                 (body: { username, password }) → { token, ... }
 *  - GET   http://localhost:8003/api/auth/me?token=TOKEN        (get current user)
 *  - GET   http://localhost:8005/questions                      (get all questions)
 *  - POST  http://localhost:8005/submit?token=TOKEN             (body: { answers: [{question_id, selected_option}] }) → { score | total_score }
 *
 * Notes:
 *  - Token is stored in localStorage so refresh keeps session.
 *  - After submit, we navigate back to Dashboard carrying the latest score in navigation state.
 *  - Styling uses Tailwind utility classes (works in this preview). If your local project has no Tailwind yet, the markup still works without styles.
 */
...
const TEST_BASE = "http://localhost:8005";
...
function AuthProvider({ children }) {
  ...
  const value = useMemo(() => ({ token, user, login, logout, loading, refreshMe }), [token, user, loading]);
...
function DashboardPage() {
  ...
  useEffect(() => {
    // Optionally refresh profile on mount (if backend provides score there)
    refreshMe();
  }, []); // eslint-disable-line
